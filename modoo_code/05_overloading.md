...
앞서 봤던 암시적 캐스팅과 명시적 캐스팅을 다시 보면 암시적 캐스팅은 그냥 컴파일러에서 알아서 캐스팅하는 것이고, 명시적 캐스팅은 직접 동작을 지정하는 캐스팅이었다.

암시적 캐스팅의 경우 `int` 와 `double` 가 덧셈을 수행할 때 `int` 형 변수가 자동으로 `double` 변수로 캐스팅되는 예시가 있다.
명시적 캐스팅의 경우 `void *` 타입의 주소를 특정 구조체 포인터 타입의 주소로 바꾸는 등의 캐스팅이 있다.
이때, 명시적 캐스팅은 다음과 같은 동작을 거친다.
...
즉, 괄호 안에 원하는 타입을 넣고 변환을 수행하는 것이다. 하지만 이러한 방식을 사용하다 보니 개발자 사이에서 몇가지 문제점들을 발견했다.

첫째로 위와 같은 타입 캐스팅의 경우, 말도 안되는 캐스팅에 대해 컴파일러 오류가 발생하지 않는다. 
둘쨰로 괄호 안에 타입을 넣는 방식으로 변환을 수행하는 탓에 가독성이 떨어진다.

```c++
function((int)variable);
```

위와 같이 괄호가 계속해서 많아지면 읽는 사람이나 코드를 유지,보수 하는 사람 입장에서 여러모로 불편한다.
마지막으로 읽는 이가 캐스팅의 의미를 명확하게 알 수 없다.

하지만 C++에서는 4가지의 캐스팅을 제공하여 이러한 문제들을 방지한다.

- `static_cast` : 우리가 흔히 생각하는, 언어적 차원에서 지원하는 일반적인 타입 변환
- `const_cast` : 객체의 상수성(const)를 없애는 타입. 쉽게 말해 `const int` 가 `int` 로 바뀐다.
- `dynamic_cast` : 파생 클래스 사이에서 다운 캐스팅(정확한 의미는 추후 나옴)
- `reinterpret_cast` : 위험을 감수하고 하는 캐스팅, 서로 관련이 없는 포인터들 사이의 캐스팅 등

이때 이러한 캐스팅을 사용하는 방법은 다음과 같다.

```c++
(원하는 캐스팅 종류)<바꾸려는 타입>(무엇을 바꿀 것인가?)
```

예를 들어, `static_cast` 로 `float` 타입의 `float_variable` 변수를 `int` 타입으로 변환하기 위해서는
...
지금까지 배운 것에선 `static_cast` 만 사용하고 나머지 캐스팅은 신경쓰지 않아도 된다.
추후 다른 캐스팅에 대해서 알아볼 것이다.
